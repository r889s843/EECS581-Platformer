...
{
    public float scrollSpeed = 100f; // Speed of the scrolling
    public RectTransform banner1;  // First banner image
    public RectTransform banner2;  // Second banner image
    public RectTransform banner3;  // Second banner image
    ...
    void Update()
    {
        ...
        banner3.anchoredPosition += new Vector2(-scrollSpeed * Time.deltaTime, 0);

        // Check if banner1 has moved off screen (left side)
        if (banner1.anchoredPosition.x <= -bannerWidth-320)
        {
            // Move banner1 to the right of banner2 to create a seamless loop
            banner1.anchoredPosition = new Vector2(banner3.anchoredPosition.x + bannerWidth, 0);
        }

        // Check if banner2 has moved off screen (left side)
        if (banner2.anchoredPosition.x <= -bannerWidth-320)
        {
        ...
        // Check if banner3 has moved off screen (left side)
        if (banner3.anchoredPosition.x <= -bannerWidth-320)
        {
            // Move banner2 to the right of banner1 to create a seamless loop
            banner3.anchoredPosition = new Vector2(banner2.anchoredPosition.x + bannerWidth, 0);
        }
    }
}
